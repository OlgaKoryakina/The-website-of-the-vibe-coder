(function () {
  ...
  window.initTestimonials = function initTestimonials() {
      ...
      cards.forEach(function (card, index) {
        ...
        if (card !== activeCard) {
          card.style.transform =
            "rotate(" + style.rotate + "deg) translateX(" + style.offset + "%)";
        }

        card.setAttribute("aria-hidden", pos === "front" ? "false" : "true");
      });
      stack.classList.remove("testimonials__stack--single");
    }
    ...
    function onPointerUp(e) {
      ...
      if (dragStartX - e.clientX > SWIPE_THRESHOLD) shuffle();
      else applyPositions();
      activeCard = null;
      dragStartX = 0;
    }
    ...
    cards.forEach(function (card) {
      ...
      if (!prefersReduced) {
        card.onclick = function () {
          if (didDrag) return;
          if (card.getAttribute("data-position") === "front") shuffle();
        };
      }
    });
